(function () {
     ...
     *     an HTML file (/angularJS/blog-admin-app/article-view/article-view.component.html), with the associated Controller in
     ...
     * - $window is used to redirect the page to 'admin#!/article-list' after successfully deleting the article
     ...
     *     article is being edited, or simply viewed without text areas on screen.
    ...
    angular.module('articleView').component('articleView', {
        templateUrl: '/angularJS/blog-admin-app/article-view/article-view.component.html',
            ...
            function articleViewController($sanitize, $window, $scope, $routeParams, ArticleIndex, ServerConfig, ViewModes, Category) {
                ...
                ArticleIndex.get(function successCallback(articles) {
                    ...
                    Category.get(function successCallback(categories) {
                        ...
                        /* Having saved the list of all categories for use in the View (in the 'Categories' side area)
                         * select the category corresponding to the currently viewed article.
                         */
                ...
                /* PUT updates to the current article, saving the new data on the server, and refreshing the local data
                 * store.
                 */
                function saveArticle(article) {
                    /* If the article is (or will be) private, check to see if all the articles in it's category are
                     * private; if they are, make sure that the category is private.
                     */
                    if (article.private && self.similarArticles.filter(function (a) {
                            return !a.private;
                        }).length == 0) {
                ...
                function deleteArticle() {
                    ArticleIndex.deleteArticle(self.article._id, function successCallback(response) {
                            /* If successful, just reroute to the article & category list, making sure to refresh the
                             * data model for both categories and articles before doing so; admins should always see
                            ...
                            /* If an error occurs, display it and don't navigate away from the page,
