(function () {
    ...
    const defaultMeta = {
            ...
            description: 'I write to investigate the Good, the Beautiful, the '
                + 'Simple, the Sublime, and the Just, in order to better '
                + 'understand what wisdom is, what a good education is, and '
                + 'what a good life is.',
    ...
    const getMetaForArticle = function (articleName, callback) {
        const correctedName = articleName.split('-').join(' ');
        Article.find().public()
                .byTitle(correctedName).exec(function (err, articles) {
            ...
            // The description should be the first sentence of the article, with
            // any wrapping HTML tags trimmed.
            // TODO: Determine whether HTML is supported in Open Graph meta tags
            ...
            callback({
                ...
                type: 'blog',
                image: 'http://www.hrodebert.com/'
                    + (article.image || 'img/favicon.png'),
                image_secure_url: 'https://www.hrodebert.com/'
                    + (article.image || 'img/favicon.png'),
                image_width: article.image_dimensions.width
                    || defaultMeta.image_width,
                image_height: article.image_dimensions.height
                    || defaultMeta.image_height,
                twitter_site: '@RobertMitchel_l',
                twitter_card: 'summary'
            });
        });
    ...
    const getMetaForCategory = function (categoryName, callback) {
        if (!categoryName) {
            categoryName = 'Everything';
        }
        Category.find().public()
                .byName(categoryName).exec(function (err, categories) {
            if (categories === []) {
                // No such category exists; send the default meta for the site.
                console.log('Couldn\'t find the desired category:',
                    categoryName);
                callback(defaultMeta);
            }
            ...
            callback({
                ...
                type: 'blog',
                image: 'http://www.hrodebert.com/'
                    + (category.image || 'img/favicon.png'),
                image_secure_url: 'https://www.hrodebert.com/'
                    + (category.image || 'img/favicon.png'),
    ...
    // GET the home page.
    // Renders the home page for the whole blog, which is distinct from the home
    // page for the admin panel. The page is formatted according to index.jade,
    // and is a completely separate AngularJS app from that of the admin panel
    // site.
    router.get('/', function (req, res, next) {
        res.render(angular2Template, defaultMeta);
    });

    // GET the about page.
    // Renders the about page with the proper meta tags for SEO.
    router.get('/about', function (req, res, next) {
        res.render(angular2Template, {
    ...
    router.get('/article-list', function (req, res, next) {
        const meta = JSON.parse(JSON.stringify(defaultMeta));
    ...
    router.get('/article-list/:category/:author', function (req, res, next) {
        getMetaForCategory(req.params.category, function (meta) {
            meta.url = 'https://www.hrodebert.com/article-list/'
                + req.params.category + '/' + req.params.author;
            res.render(angular2Template, meta);
        });
    });
    ...
    router.get('/article-list/:category/:startDate/:endDate',
        function (req, res, next) {
            getMetaForCategory(req.params.category, function (meta) {
                meta.url = 'https://www.hrodebert.com/article-list/'
                    + req.params.category + '/' + req.params.startDate + '/'
                    + req.params.endDate;
                res.render(angular2Template, meta);
            });
        }
    ...
    // GET the article-list page with default filters.
    // Renders the article-list page with the proper meta tags for SEO.
    router.get('/article-list/:category/:authorName/:startDate/:endDate',
        function (req, res, next) {
            getMetaForCategory(req.params.category, function (meta) {
                meta.url = 'https://www.hrodebert.com/article-list/'
                    + req.params.category + '/' + req.params.authorName + '/'
    ...
    // GET the admin page.
    // Renders the home page for the admin panel, which is distinct from the
    // home page for the normal blog. The page is formatted according to
    // admin.jade, and is a completely separate AngularJS app from that of the
    // public site.
